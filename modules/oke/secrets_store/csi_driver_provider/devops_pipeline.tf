  ...
  charts = [
    {
      ...
      "chart_name"    = "oci-secret-store-provider"
...
module "secrets_store_csi_provider_chart_devops_artifact" {
  ...
  chart_url     = "oci://${var.region}.ocir.io/${local.namespace}/${var.deploy_id}/secrets_store_csi_provider/secrets_store_csi_provider"
  chart_version = var.secrets_store_csi_provider_helm_chart_version
  project_id    = var.devops_project_id
  defined_tags  = var.defined_tags
}
...
resource "oci_devops_deploy_stage" "secrets_store_csi_provider" {
  ...
  release_name                      = "secrets_store_csi_provider"
  rollback_policy {
