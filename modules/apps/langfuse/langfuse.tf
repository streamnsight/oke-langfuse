resource "null" "install_langfuse" {
    ...
    provisioner "local-exec" {
        command = "./create_langfuse_secrets.sh"
        # Optional arguments:
        when        = "create"
        on_failure  = "fail"   # or "continue"
        environment = {
          ...
          IDCS_CLIENT_SECRET = ""
          IDCS_HOST = ""
          PASSWORD_ENCRYPTION_KEY = random_string.password_encryption_key
          PASSWORD_ENCRYPTION_SALT = random_string.password_encryption_salt
          NEXTAUTH_SECRET = random_string.nextauth_secret
          CLICKHOUSE_PASSWORD = random_string.clickhouse_password
          LANGFUSE_K8S_NAMESPACE = "langfuse"
        }
    }
