...
locals {
  devops_policy_statements = [
    ...
    "allow any-user to manage secret-family in compartment id ${var.devops_compartment_id} where all {request.principal.type = 'devopsdeploypipeline', target.compartment.id = '${var.devops_compartment_id}'}",
    "allow any-user to read postgres-db-system in compartment id ${var.devops_compartment_id} where all {request.principal.type = 'devopsdeploypipeline', target.compartment.id = '${var.devops_compartment_id}'}",
  ]
}
